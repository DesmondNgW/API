                ...
                foreach (XmlNode item in node.ChildNodes)
                {
                    switch (item.Name)
                    {
                        case "urls":
                            ...
                            break;
                        case "socketPool":
                            couchbase.MinPoolSize = XmlHelper.GetXmlAttributeValue(item, "minPoolSize", string.Empty).Convert2Int32(20);
                            couchbase.MaxPoolSize = XmlHelper.GetXmlAttributeValue(item, "maxPoolSize", string.Empty).Convert2Int32(1000);

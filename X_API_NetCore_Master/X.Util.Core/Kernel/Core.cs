...
namespace X.Util.Core.Kernel
{
    public class Core<T>
    {
        ...
        public static T Instance(Func<string, string, string, T> init, string argument1, string argument2, string argument3, string cacheKey, TimeSpan validTime, Func<T, bool> validState = null)
        {
            var key = cacheKey + typeof(T).FullName;
            if (ContainsCache(key, validState)) return _cacheResult[key].Result;
            lock (CoreUtil.Getlocker(LockerPrefix + key))
            {
                if (!ContainsCache(key, validState))
                    _cacheResult[key] = new StatusCacheResult<T>
                    {
                        Result = init(argument1, argument2, argument3),
                        CacheKey = key,
                        ExpiryDate = DateTime.Now.Add(validTime)
                    };
            }
        ...
        public static T Instance(Func<string, string, string, T> init, string argument1, string argument2, string argument3, string cacheKey, Func<T, bool> validState = null)
        {
            return Instance(init, argument1, argument2, argument3, cacheKey, new TimeSpan(DayOfHour, 0, 0), validState);
        }

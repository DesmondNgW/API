...
namespace X.Util.Core.Kernel
{
    public class CoreChannel<T> : ChannelFactory<T>
    {
        ...
        protected override ServiceEndpoint CreateDescription()
        {
            ...
            foreach (ChannelEndpointElement endpoint in group.Clients.Endpoints)
            {
                ...
                #region [[endpoint unsupport: Identity && Headers]] 
                //#new EndpointAddress(endpointAddress, ServiceModelTool.GetIdentity(endpoint.Identity), endpoint.Headers.Headers);
                #endregion
                if (Equals(serviceEndpoint.Address, null)) serviceEndpoint.Address = new EndpointAddress(endpointAddress);
                if (serviceEndpoint.EndpointBehaviors.Count.Equals(0) && !string.IsNullOrEmpty(endpoint.BehaviorConfiguration)) ServiceModelTool.AddBehaviors(endpoint.BehaviorConfiguration, serviceEndpoint, @group);

...
namespace NetSerializer.TypeSerializers
{
    sealed class GenericSerializer : IDynamicTypeSerializer
    {
        ...
        {
            if (!type.IsSerializable) throw new NotSupportedException(string.Format("Type {0} is not marked as Serializable", type.FullName));
            if (typeof(System.Runtime.Serialization.ISerializable).IsAssignableFrom(type)) throw new NotSupportedException(string.Format("Cannot serialize {0}: ISerializable not supported", type.FullName));
            return true;
        }
        ...
        public void GenerateWriterMethod(Serializer serializer, Type type, ILGenerator il)
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (data.WriterNeedsInstance) il.Emit(OpCodes.Ldarg_0);
        ...
        public void GenerateReaderMethod(Serializer serializer, Type type, ILGenerator il)
        {
            ...
            foreach (var field in fields)
            {
                var fieldType = field.FieldType;
                var data = serializer.GetIndirectData(fieldType);
                if (data.ReaderNeedsInstance) il.Emit(OpCodes.Ldarg_0);
                il.Emit(OpCodes.Ldarg_1);
                il.Emit(OpCodes.Ldarg_2);
                if (type.IsClass) il.Emit(OpCodes.Ldind_Ref);
            ...
            {
                var miOnDeserialization = typeof(System.Runtime.Serialization.IDeserializationCallback).GetMethod("OnDeserialization", BindingFlags.Instance | BindingFlags.Public, null, new[] { typeof(object) }, null);
                il.Emit(OpCodes.Ldarg_2);

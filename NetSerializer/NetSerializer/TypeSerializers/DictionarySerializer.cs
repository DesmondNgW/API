...
namespace NetSerializer.TypeSerializers
{
    sealed class DictionarySerializer : IStaticTypeSerializer
    {
        ...
        static MethodInfo GetGenWriter(IReflect containerType, Type genType)
        {
            var mis = containerType.GetMethods(BindingFlags.Static | BindingFlags.Public).Where(mi => mi.IsGenericMethod && mi.Name == "WritePrimitive");
            return (from mi in mis let p = mi.GetParameters() where p.Length == 3 where p[1].ParameterType == typeof (Stream) let paramType = p[2].ParameterType where paramType.IsGenericType let genParamType = paramType.GetGenericTypeDefinition() where genType == genParamType select mi).FirstOrDefault();
        }

        static MethodInfo GetGenReader(IReflect containerType, Type genType)
        {
            var mis = containerType.GetMethods(BindingFlags.Static | BindingFlags.Public).Where(mi => mi.IsGenericMethod && mi.Name == "ReadPrimitive");
            foreach (var mi in mis)
            {
                var p = mi.GetParameters();
                if (p.Length != 3) continue;
                if (p[1].ParameterType != typeof(Stream)) continue;
                var paramType = p[2].ParameterType;
                if (paramType.IsByRef == false) continue;
                paramType = paramType.GetElementType();
                if (paramType.IsGenericType == false) continue;
                var genParamType = paramType.GetGenericTypeDefinition();
                if (genType == genParamType) return mi;
            }
            return null;
        }
        ...
        public static void WritePrimitive<TKey, TValue>(Serializer serializer, Stream stream, Dictionary<TKey, TValue> value)
        {
            ...
            var i = 0;
            foreach (var kvp in value) kvpArray[i++] = kvp;
        ...
        public static void ReadPrimitive<TKey, TValue>(Serializer serializer, Stream stream, out Dictionary<TKey, TValue> value)
        {
            ...
            foreach (var kvp in kvpArray) value.Add(kvp.Key, kvp.Value);
        }
    }
}
